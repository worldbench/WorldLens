...
def process_video(token, video_root, gen_idx, key_frame_index, view_order, 
    ...
    clip_tokens = token_info_dict[token]['clip_tokens']
    for i in range(len(key_frame_index)):
        this_kf_info = deepcopy(token_info_dict[clip_tokens[i]])
        this_kf_info['scene_token'] = clip_tokens[i] + f"_gen{gen_idx}"
        vid_infos.append(this_kf_info)

    # handle each view
    # for view in view_order:
    for view in tqdm(view_order, desc=f"Processing video {token}"):
...
def decode_video(args):
    ...
    key_frame_index = [i for i in range(16)]
    out_size = (1600, 900)
    ...
    token_info_dict = {}
    first_frame_tokens = [token_list[0] for token_list in data['scene_tokens']]
    for info in data['infos']:
        if info['token'] in first_frame_tokens:
            index = first_frame_tokens.index(info['token'])
            info['clip_tokens'] = data['scene_tokens'][index][:16]
        token_info_dict[info['token']] = info
    print(f"loaded scenes: {len(first_frame_tokens)}")

    save_paths = []
    for gen_idx in range(args.generation_times):
        save_dir = os.path.join('generated_results', args.model_name, f"full_gen_samples_{gen_idx}")
        ...
        # just for test
        # process_video(first_frame_tokens[0], args.vid_root, gen_idx, key_frame_index, view_order, token_info_dict, out_size, save_dir)
        ...
        save_path = os.path.join('generated_results', args.model_name, f"nuscenes_infos_temporal_val_fullframes_gen{gen_idx}.pkl")
