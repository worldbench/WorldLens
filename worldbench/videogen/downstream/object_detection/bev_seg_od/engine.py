...
sys.path.append('worldbench/third_party/BEVFormer_segmentation_detection')
...
from projects.mmdet3d_plugin.datasets.builder_dataset import build_dataset
...
def parse_args(arg_list=None):
    ...
    parser.add_argument('--config', 
                        default='worldbench/third_party/BEVFormer_segmentation_detection/projects/configs/bevformer/bevformer_base_seg_det_150x150.py',
                        help='test config file path')
    parser.add_argument('--checkpoint', 
                        default='pretrained_models/perception/bevformer_base_seg_det_150.pth',
...
def run(args):
    ...
    log_root = osp.join(
        f'generated_results/{args.method_name}/bev_seg_od', args.config.split('/')[-1].split('.')[-2],
        args.log_timestamp,
    )
    ...
    if not distributed:
        ...
        outputs = multi_gpu_test(
            model, data_loader, args.tmpdir, args.gpu_collect, log_root)
    breakpoint()
    if rank == 0:
        if args.out:
        ...
        kwargs['jsonfile_prefix'] = log_root
        if args.format_only:
...
if __name__ == '__main__':
    # https://github.com/open-mmlab/mmdetection3d/issues/1364
