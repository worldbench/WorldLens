...
from .dreamforge_nusc_dataset import DFNuScenesDataset

@DATASETS.register_module()
class NuScenesMapDataset(DFNuScenesDataset):
    def __init__(
        self,
        ann_file,
        pipeline=None,
    ...
    def load_annotations(self, ann_file):
        ...
        for i in range(len(data_infos)):
            ori_lidar_path = data_infos[i]['lidar_path']
            new_path = ori_lidar_path.replace('/cpfs01/shared/public/ADLab/datasets', './data')
            data_infos[i]['lidar_path'] = new_path

            for cam_id, cam_info in data_infos[i]['cams'].items():
                ori_cam_path = cam_info['data_path']
                new_cam_path = ori_cam_path.replace('/cpfs01/shared/public/ADLab/datasets', './data')
                data_infos[i]['cams'][cam_id]['data_path'] = new_cam_path
