...
def get_interp_novel_trajectories(
    ...
    trajectory_generators = {
        ...
        "lateral_offset": lateral_offset_trajectory,
    }
...
def lateral_offset_trajectory(
    ...
    """
    Translate the default reference camera trajectory sideways by a fixed amount.

    Args:
        dataset_type: Dataset name to select the default camera.
        per_cam_poses: Original camera poses keyed by camera id.
        original_frames: Number of frames in the original recording.
        target_frames: Desired length of the interpolated trajectory.
        offset_distance: Lateral offset in meters (positive = right, negative = left).

    Returns:
        torch.Tensor: Interpolated poses with lateral offset applied.
    """
    if len(per_cam_poses) == 0:
        raise ValueError("per_cam_poses is empty, cannot build lateral offset trajectory")

    # Select the same default camera used for front-center renderings.
    if dataset_type == "argoverse":
        default_cam = 1 if 1 in per_cam_poses else 0
    else:
        default_cam = 0 if 0 in per_cam_poses else list(per_cam_poses.keys())[0]

    base_poses = per_cam_poses[default_cam].clone()
    lateral_axis = base_poses[:, :3, 0]
    base_poses[:, :3, 3] = base_poses[:, :3, 3] + lateral_axis * offset_distance

    return interpolate_poses(base_poses, target_frames)
