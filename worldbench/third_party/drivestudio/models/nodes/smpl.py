...
class SMPLNodes(RigidNodes):
    ...
    def get_gaussians(self, cam: dataclass_camera) -> Dict[str, torch.Tensor]:
        ...
        if self.sh_degree > 0:
            ...
            viewdirs_norm = viewdirs.norm(dim=-1, keepdim=True)
            viewdirs = viewdirs / viewdirs_norm.clamp_min(1e-6)
            zero_dir_mask = viewdirs_norm.squeeze(-1) < 1e-6
            if zero_dir_mask.any():
                viewdirs[zero_dir_mask] = torch.tensor([0.0, 0.0, 1.0], device=viewdirs.device)
