...
class RigidNodes(VanillaGaussians):
    ...
    def create_from_pcd(self, instance_pts_dict: Dict[str, torch.Tensor]) -> None:
        ...
        distances_np, _ = k_nearest_sklearn(self._means.data, 3)
        distances = torch.from_numpy(distances_np)
        if distances.shape[1] > 0:
            avg_dist = distances.mean(dim=-1, keepdim=True).to(self.device)
            scales_init = avg_dist.expand(-1, 3).clone()
        else:
            default_scale = 1e-3
            scales_init = torch.full((self.num_points, 3), default_scale, device=self.device)
        scales_init = scales_init.clamp(0.002, 100)
        self._scales = Parameter(torch.log(scales_init))
    ...
    def refinement_after(self, step: int, optimizer: torch.optim.Optimizer) -> None:
        ...
        with torch.no_grad():
            ...
            if do_densification:
                ...
                # Keep mask 1D; avoid 0-D when num_points==1
                ...
                if self.step < self.ctrl_cfg.stop_screen_size_at:
                    splits = splits | (self.max_2Dsize > self.ctrl_cfg.split_screen_size).reshape(-1)
                splits = splits & high_grads
    ...
    def split_gaussians(self, split_mask: torch.Tensor, samps: int = 2) -> Tuple:
        ...
        """
        # Normalize mask shape to 1D to avoid 0-D scalars when N == 1
        if split_mask.dim() != 1:
            split_mask = split_mask.reshape(-1)

        n_splits = int(split_mask.sum().item())
        print(f"    Split: {n_splits}")
        # Early return on zero-split to avoid downstream ops on empty tensors
        if n_splits == 0:
            device = self.device
            return (
                torch.empty((0, self._means.shape[-1]), device=device, dtype=self._means.dtype),
                torch.empty((0, self._features_dc.shape[-1]), device=device, dtype=self._features_dc.dtype),
                torch.empty((0, self._features_rest.shape[1], self._features_rest.shape[-1]), device=device, dtype=self._features_rest.dtype),
                torch.empty((0, self._opacities.shape[-1]), device=device, dtype=self._opacities.dtype),
                torch.empty((0, self._scales.shape[-1]), device=device, dtype=self._scales.dtype),
                torch.empty((0, self._quats.shape[-1]), device=device, dtype=self._quats.dtype),
                torch.empty((0, self.point_ids.shape[-1]), device=device, dtype=self.point_ids.dtype),
            )

        centered_samples = torch.randn((samps * n_splits, 3), device=self.device)  # Nx3 of axis-aligned scales
        # Normalize shapes to 2D along the last dim, preserving order
        scales_selected_raw = self._scales[split_mask]
        scales_selected = torch.exp(scales_selected_raw.reshape(-1, scales_selected_raw.shape[-1]))  # (n_splits, C)
        scaled_samples = (
            scales_selected.repeat(samps, 1) * centered_samples
        )  # how these scales are rotated
        quats_raw = self._quats[split_mask]
        quats = self.quat_act(quats_raw.reshape(-1, quats_raw.shape[-1]))  # normalize them first, (n_splits, 4)
        rots = quat_to_rotmat(quats.repeat(samps, 1))  # (samps*n_splits, 3, 3)
        rotated_samples = torch.bmm(rots, scaled_samples[..., None]).squeeze()
        means_raw = self._means[split_mask]
        new_means = rotated_samples + means_raw.reshape(-1, means_raw.shape[-1]).repeat(samps, 1)
        # step 2, sample new colors
        # new_colors_all = self.colors_all[split_mask].repeat(samps, 1, 1)
        new_feature_dc = self._features_dc[split_mask].reshape(-1, self._features_dc.shape[-1]).repeat(samps, 1)
        new_feature_rest = self._features_rest[split_mask].repeat(samps, 1, 1)
        # step 3, sample new opacities
        new_opacities = self._opacities[split_mask].reshape(-1, self._opacities.shape[-1]).repeat(samps, 1)
        # step 4, sample new scales
        size_fac = 1.6
        _sel_scales = self._scales[split_mask]
        new_scales = torch.log(torch.exp(_sel_scales) / size_fac).reshape(-1, _sel_scales.shape[-1]).repeat(samps, 1)
        self._scales[split_mask] = torch.log(torch.exp(self._scales[split_mask]) / size_fac)
        # step 5, sample new quats
        new_quats = self._quats[split_mask].reshape(-1, quats_raw.shape[-1]).repeat(samps, 1)
        # step 6, sample new ids
        new_ids = self.point_ids[split_mask].reshape(-1, self.point_ids.shape[-1]).repeat(samps, 1)
    ...
    def transform_means(self, means: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Normalize input shapes for robustness when N==1
        if means.dim() == 1:
            # Accept (3,) → (1,3)
        ...
        if rot_per_pts.dim() == 2:
            # Handle degenerate case when advanced indexing returns (3,3)
            rot_per_pts = rot_per_pts.unsqueeze(0)
        ...
        if trans_per_pts.dim() == 1:
            trans_per_pts = trans_per_pts.unsqueeze(0)
    ...
    def transform_quats(self, quats: torch.Tensor) -> torch.Tensor:
        ...
        according to the pose at the current frame
        """
        # Normalize shapes for robustness when N==1
        if quats.dim() == 1:
            quats = quats.reshape(1, -1)
        else:
            quats = quats.reshape(-1, quats.shape[-1])
        ...
        if global_quats_per_pts.dim() == 1:
            global_quats_per_pts = global_quats_per_pts.unsqueeze(0)
