...
def render_images(
    ...
    save_image_pairs: Optional[bool] = False
):
    """
    ...
    render_results = render(
        ...
        save_image_pairs=save_image_pairs
    )
    if compute_metrics:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            ...
            if compute_metrics:
                ...
                if save_image_pairs:
                    save_path = 'image_pairs'
                    os.makedirs(os.path.join(save_path, 'gt'), exist_ok=True)
                    os.makedirs(os.path.join(save_path, 'pred'), exist_ok=True)
                    Image.fromarray((get_numpy(rgb)*255).astype('uint8')).save(f'{save_path}/pred/{i}.png')
                    Image.fromarray((get_numpy(image_infos["pixels"])*255).astype('uint8')).save(f'{save_path}/gt/{i}.png')
