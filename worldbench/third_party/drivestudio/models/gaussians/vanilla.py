...
class VanillaGaussians(nn.Module):
    ...
    def create_from_pcd(self, init_means: torch.Tensor, init_colors: torch.Tensor) -> None:
        ...
        target_dim = 1 if self.ball_gaussians else (2 if self.gaussian_2d else 3)

        if distances.shape[1] > 0:
            avg_dist = distances.mean(dim=-1, keepdim=True).to(self.device)
            scales_init = avg_dist.expand(-1, target_dim).clone()
        else:
            # fallback for extremely sparse point clouds
            default_scale = 1e-3
            scales_init = torch.full((self.num_points, target_dim), default_scale, device=self.device)
