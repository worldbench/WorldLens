...
def k_nearest_sklearn(x: torch.Tensor, k: int):
    """
    Find the k nearest neighbors (excluding the query point).

    Returns arrays of shape (num_samples, effective_k) where
    effective_k = min(k, num_samples - 1). Downstream code should
    handle the case effective_k < k.
    """
    num_samples = x.shape[0]
    effective_k = min(k, max(num_samples - 1, 0))
    if effective_k == 0:
        return np.zeros((num_samples, 0), dtype=np.float32), np.empty((num_samples, 0), dtype=np.float32)
    ...
    indices = indices[:, 1:].astype(np.float32)
