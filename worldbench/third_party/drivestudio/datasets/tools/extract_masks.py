...
if __name__ == "__main__":
    ...
    if args.scene_ids is not None:
        ...
        scene_ids_list = sorted(os.listdir(args.data_root))[:args.num_scenes]
    ...
    for scene_i, scene_id in enumerate(tqdm(scene_ids_list, f'Extracting Masks ...')):
        ...
        wait_flag = False
        while not os.path.exists(os.path.join(args.data_root, scene_id, "instances", "instances_info.json")):
            if not wait_flag:
                print(f"Scene {scene_id} has less than 16*6 images, waiting ...")
            wait_flag = True
        wait_flag = False
        ...
        for fpath in tqdm(flist, f'scene[{scene_id}]'):
            ...
            if args.ignore_existing and os.path.exists(os.path.join(args.data_root, scene_id, "fine_dynamic_masks")) and \
                len(os.listdir(os.path.join(args.data_root, scene_id, 'fine_dynamic_masks', 'all'))) == len(os.listdir(os.path.join(args.data_root, scene_id, 'images'))) and \
                len(os.listdir(os.path.join(args.data_root, scene_id, 'fine_dynamic_masks', 'human'))) == len(os.listdir(os.path.join(args.data_root, scene_id, 'images'))) and \
                len(os.listdir(os.path.join(args.data_root, scene_id, 'fine_dynamic_masks', 'vehicle'))) == len(os.listdir(os.path.join(args.data_root, scene_id, 'images'))):
