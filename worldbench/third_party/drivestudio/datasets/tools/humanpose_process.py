...
if __name__ == "__main__":
    ...
    if args.scene_ids is not None:
        ...
        scene_ids_list = sorted(os.listdir(args.data_root))
        scene_ids_list = scene_ids_list[:args.num_scenes]
    ...
    for scene_id in scene_ids_list:
        ...
        try:
            ...
            wait_flag = False
            while not os.path.exists(os.path.join(scene_dir, "instances", "instances_info.json")):
                if not wait_flag:
            ...
            # check exists
            if os.path.exists(os.path.join(scene_dir, 'humanpose', 'smpl.pkl')):
                print(f"Scene {scene_id} already processed, skip it.")
                continue
