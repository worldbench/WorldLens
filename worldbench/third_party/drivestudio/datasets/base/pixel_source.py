...
class ScenePixelSource(abc.ABC):
    ...
    def prepare_novel_view_render_data(self, dataset_type: str, traj: torch.Tensor, cam_id: int = None) -> list:
        ...
        Args:
            ...
            cam_id (int, optional): Specific camera ID to use for intrinsics and image size.
                If None, keep dataset default behavior (Argoverse uses 1, others use 0).
        ...
        if cam_id is None:
            if dataset_type == "argoverse":
                cam_id = 1  # default for Argoverse
            else:
                cam_id = 0  # default for other datasets
        # ensure the selected camera exists in current configuration
        if cam_id not in self.camera_list:
            raise ValueError(f"Requested camera id {cam_id} is not in active camera list {self.camera_list}")
        ...
        for i in range(len(traj)):
            ...
            image_infos = {
                ...
                "viewdirs": viewdirs,
                "direction_norm": direction_norm,
                # keep per-video local indexing for appearance embeddings; videos are saved per-cam
