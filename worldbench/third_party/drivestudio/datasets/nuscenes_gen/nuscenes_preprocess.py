...
def read_mp4_to_ndarray_list(video_path: str, target_size=None):
    ...
    while True:
        ...
        frames.append(frame)  # frame 默认是 np.ndarray，形状 (H, W, 3)
...
class NuScenesProcessor(object):
    ...
    def __init__(
        ...
        post_fix = f"_{(interpolate_N+1)*2}Hz_{self.data_source}" if interpolate_N > 0 else f"_{self.data_source}"
        ...
        for scene_idx in range(len(self.nusc.scene)):
            ...
            if scene['name'] in splits.val:
                _process_id_list.append(scene_idx)
        self.process_id_list = _process_id_list[:num_scenes]
        self.create_folder()
    ...
    def convert_one(self, scene_idx):
        ...
        for camera in self.cam_list:
            if self.data_source == "drivedreamer2":
                # TODO: note that the resolution of drivedreamer2 is [1600, 896], we assume directly reshape it to [1600, 900] is acceptable
                images_single = read_mp4_to_ndarray_list(f'data/nuscenes_gen/{self.data_source}/video_submission/{scene_data["first_sample_token"]}_gen01/{scene_data["first_sample_token"]}_{camera}.mp4', target_size=(1600, 900))
            elif self.data_source in ("dist4d", "opendwm", "xscene"):
                images_single = read_mp4_to_ndarray_list(f'data/nuscenes_gen/{self.data_source}/video_submission/{scene_data["first_sample_token"]}_gen0/{scene_data["first_sample_token"]}_{camera}.mp4')
            else:
                images_single = read_mp4_to_ndarray_list(f'data/nuscenes_gen/{self.data_source}/video_submisson/{scene_data["first_sample_token"]}_gen0/{scene_data["first_sample_token"]}_{camera}.mp4')
            images[camera] = dict(zip(sample_tokens[:len(images_single)], images_single))
        max_frame_idx = len(images_single) - 1

        selected_tokens = sample_tokens[:len(images_single)]
        if "images" in self.process_keys:
            for key_frame_idx in range(len(images_single)):
                for cam_idx, cam_name in enumerate(self.cam_list):
                    ...
                    cv2.imwrite(img_path, images[cam_name][selected_tokens[key_frame_idx]])
        ...
        if "calib" in self.process_keys:
            self.save_calib(scene_data, scene_idx, max_frame_idx)
            print(f"Processed calib for scene {str(scene_idx).zfill(3)}")
        if "lidar" in self.process_keys:
            self.save_lidar(scene_data, scene_idx, max_frame_idx)
            print(f"Processed lidar for scene {str(scene_idx).zfill(3)}")
        if "dynamic_masks" in self.process_keys:
            self.save_dynamic_mask(scene_data, scene_idx, max_frame_idx, class_valid='all')
            self.save_dynamic_mask(scene_data, scene_idx, max_frame_idx, class_valid='human')
            self.save_dynamic_mask(scene_data, scene_idx, max_frame_idx, class_valid='vehicle')
            print(f"Processed dynamic masks for scene {str(scene_idx).zfill(3)}")

        # process annotated objects
        if "objects" in self.process_keys:
            instances_info, frame_instances = self.save_objects(scene_data, max_frame_idx)
    ...
    def save_calib(self, scene_data, scene_idx, max_frame_idx):
        ...
        while True:
            ...
            if curr_sample_record['next'] == '' or curr_sample_record['token'] == last_sample_token or key_frame_idx == max_frame_idx:
                break
            key_frame_idx += 1
            curr_sample_record = self.nusc.get('sample', curr_sample_record['next'])
    ...
    def save_lidar(self, scene_data, scene_idx, max_frame_idx):
    ...
    def save_dynamic_mask(self, scene_data, scene_idx, max_frame_idx, class_valid='all', img_shape=(900, 1600)):
        ...
        while True:
            for cam_idx, cam_name in enumerate(self.cam_list):
                ...
                dynamic_mask = np.zeros(img_shape[:2], dtype=np.float32)
                ...
                for ann in valid_anns:
                    ...
                    in_image = np.all(corners_2d[0, :] >= 0) & np.all(corners_2d[0, :] < img_shape[1]) & \
                            np.all(corners_2d[1, :] >= 0) & np.all(corners_2d[1, :] < img_shape[0])
                    ...
                    u = np.clip(u, 0, img_shape[1] - 1)
                    v = np.clip(v, 0, img_shape[0] - 1)
    ...
    def save_objects(self, scene_data, max_frame_idx):
